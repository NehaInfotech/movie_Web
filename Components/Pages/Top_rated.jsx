...
function TopRated({ favorites, addFavorite }) {
  return (
    <Box  sx={{ mt: 5 ,ml:{lg:7,md:4}}}>
      <Grid container spacing={2} justifyContent="center" alignItems="center">
        <Grid item xs={12}>
          <Typography sx={{display:"flex",alignItems:"center",justifyContent:"center",mt:5}} variant="h4" component="h2" gutterBottom>
            Top Rated Movies
          </Typography>
        </Grid>
        <Grid item xs={12}>
          ...
          <Grid container spacing={2}>
            {movieData.map((movie, index) => (
              <Grid item key={index} xs={12} sm={6} md={4} lg={3}>
               <Card sx={{ maxWidth: { xs: '100%', sm: '300px', md: '345px' }, height: '100%' }}>
                  <CardMedia
                    component="img"
                    height="300"
                    image={movie.image}
